...
export default function HostPage() {
  ...
  // check auth (always 200)
  useEffect(() => {
    ;(async () => {
      const res = await fetch("/api/host/me", { method: "GET", credentials: "include" })
      const j = await res.json().catch(() => ({ loggedIn: false }))
      setAuthed(!!j.loggedIn)
      setChecking(false)
      if (j.loggedIn) await refreshHostState()
    })()
